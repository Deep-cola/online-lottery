drop database if exists online_lottery;
create database online_lottery character set utf8mb4;
...
create table member(
    ...
    setting_id int not null comment '抽奖设置 id',-- setting_id，===> 以便于扩展需要
    create_time timestamp default NOW() comment '创建时间',
    foreign key (setting_id) references setting(id)
) comment '抽奖人员';
...
create table record(
    id int primary key auto_increment,
    member_id int not null comment '中奖人员 id',
    award_id int not null comment '中奖奖项 id',
    create_time timestamp default NOW() comment '创建时间',
